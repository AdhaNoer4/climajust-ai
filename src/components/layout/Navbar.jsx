    ...
    <div className="max-w-7xl mx-auto px-4 py-3">
      <div className="bg-white/80 backdrop-blur-md rounded-full shadow flex items-center justify-between px-6 py-3">
        ...
        <nav className="hidden md:flex gap-20 text-sm">
          ...
          <NavLink to="/peta-risiko" className={({ isActive }) => `relative pb-1 ${isActive ? "font-medium text-sky-600 after:absolute after:left-0 after:-bottom-1 after:h-[2px] after:w-full after:bg-sky-600" : "text-gray-700 hover:text-sky-600"}`}>
            Peta Risiko
          </NavLink>
          <NavLink to="/laporan" className={({ isActive }) => `relative pb-1 ${isActive ? "font-medium text-sky-600 after:absolute after:left-0 after:-bottom-1 after:h-[2px] after:w-full after:bg-sky-600" : "text-gray-700 hover:text-sky-600"}`}>
            Laporan
          </NavLink>
        </nav>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-3 relative w-72">
            ...
            />
            {input && (
              <button type="button" onClick={() => { setInput(""); setSuggestions([]); }}
            ...
            {suggestions.length > 0 && (
              <div className="absolute top-10 left-0 w-full bg-white rounded-xl shadow-lg z-50 overflow-hidden">
                {suggestions.map((loc) => (
                  <button key={loc.adm4_code} onClick={() => { onSelectCity(loc.name); setInput(""); setSuggestions([]); }}
            ...
            <button type="button" onClick={handleSearch}
              className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-500 hover:text-sky-600 transition">
          ...
          {user ? (
            <div className="flex items-center gap-2">
              ...
              <button onClick={onLogout}
                className="px-4 py-2 rounded-full border border-sky-600 text-sky-600 text-sm hover:bg-sky-50">
                Keluar
              </button>
            </div>
          ) : (
