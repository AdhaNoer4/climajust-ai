...
export default function CityDetailSidebar({ weatherData, metadata }) {
  if (!weatherData || !metadata) {
    return (
      <aside className="bg-white rounded-2xl shadow p-4 text-sm text-slate-500">
  ...
  const highRiskAlerts = todayForecast.filter(
    (item) =>
      item.description.toLowerCase().includes("hujan lebat") ||
      item.description.toLowerCase().includes("angin kencang")
  );

  const mediumRiskAlerts = todayForecast.filter(
    (item) =>
      item.description.toLowerCase().includes("hujan") &&
      !item.description.toLowerCase().includes("lebat")
  );
    ...
    <aside className="space-y-4">
      ...
      <section className="bg-white rounded-2xl shadow p-4 space-y-2">
        ...
        <p className="text-sm text-slate-600">
          Provinsi: {weatherData.location?.province}
        </p>
        <p className="text-sm text-slate-600">
          Kota: {weatherData.location?.city}
        </p>
        <p className="text-sm text-slate-600">
          Kecamatan: {weatherData.location?.district}
        </p>
        ...
        {weatherData.population && (
          <div className="mt-2 p-2 bg-blue-50 rounded-lg">
            ...
            <p className="text-lg font-bold text-blue-900">
              {formatNumber(weatherData.population.total)}{" "}
              {weatherData.population.satuan || "Jiwa"}
            </p>
            <p className="text-xs text-blue-600 flex items-center gap-1">
              📊 Data {weatherData.population.sumber}{" "}
              {weatherData.population.tahun}
              {weatherData.population.label && (
        ...
        <p className={`text-sm font-medium ${riskLabelColor[riskLevel]}`}>
          Level Risiko: {riskLabel[riskLevel]}
        </p>
      </section>

      {/* Ringkasan Cuaca */}
      <section className="bg-white rounded-2xl shadow p-4 grid grid-cols-3 gap-3 text-center">
        <WeatherMini label="Suhu" value={weather.temp} icon="🌡️" />
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500">Cuaca</p>
          <p className="font-semibold text-slate-800 text-sm">
            {weather.description}
          </p>
        </div>
        <WeatherMini label="Angin" value={weather.wind} icon="🌬️" />
      </section>

      {/* Peringatan Cuaca Per Jam */}
      <section className="bg-white rounded-2xl shadow p-4 space-y-3">
        <h4 className="font-semibold text-sm text-slate-800">
          ⏰ Peringatan Cuaca Hari Ini
        </h4>
        ...
        {highRiskAlerts.length === 0 && mediumRiskAlerts.length === 0 && (
          <div className="text-center py-4 text-sm text-slate-500">
            Tidak ada peringatan cuaca untuk hari ini
          </div>
        )}
      </section>
    </aside>
...
function HourlyAlert({ time, desc, level, icon }) {
  const color = {
  ...
  return (
    <div
      className={`flex items-center justify-between px-3 py-2 rounded-lg text-xs ${color[level]}`}
    >
